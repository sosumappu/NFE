...
- `packages/nfe-runtime`: deterministic `Pipeline::step`, bounded mapping worker, start-line debouncer, sync run-loop traits, runtime config, and tuning helpers that use the real pipeline path.
...
- Tuning search spaces are generated from `Tunable` descriptors; `nfe-runtime::tuning` evaluates episodes through the same pipeline used by runtime.
