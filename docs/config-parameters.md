...
| `control.perception.apex.prefer_nearer_opposite` | `true` | Select the opposite bounding point closest to the apex range when interpolation is rejected. | Disable only to compare against the older farther-point behavior in replay or controlled tests. |
| `control.perception.apex.wall_clearance_m` | `0.15` | Clearance margin subtracted from the opposite-wall target before taking the apex midpoint. | Increase if the car hugs the outside wall on corner exit; reduce if it cuts too aggressively into open space. |
| `control.perception.apex.apex_switch_threshold_rad` | `0.35` | Angular hysteresis threshold for accepting a new apex candidate. | Raise to hold targets longer through noisy mid-corner scans; lower if apex changes feel delayed. |
| `control.perception.apex.apex_switch_hysteresis_factor` | `1.8` | Score ratio required for a new apex to override hysteresis. | Increase to reject transient next-apex snaps; decrease when stronger discontinuities should be adopted sooner. |
