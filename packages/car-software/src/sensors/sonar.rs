...
pub const SONAR_SENSORS: [SonarConfig; 3] = [
    SonarConfig { label: "front",       trig_bcm: 23, echo_bcm: 24, slot: 0, startup_delay: Duration::from_millis(0)  },
    SonarConfig { label: "front-left",  trig_bcm: 25, echo_bcm:  8, slot: 1, startup_delay: Duration::from_millis(10) },
    SonarConfig { label: "front-right", trig_bcm:  7, echo_bcm:  1, slot: 2, startup_delay: Duration::from_millis(20) },
];
    ...
    SONAR_SENSORS
        ...
        .filter_map(|(cfg, ready)| {
            let trig = match gpio.get(cfg.trig_bcm) {
                Ok(p) => p.into_output(),
                Err(e) => {
                    warn!("sonar[{}]: cannot open TRIG pin BCM{} ({e}) — sensor skipped",
            ...
            let echo = match gpio.get(cfg.echo_bcm) {
                Ok(p) => p.into_input(),
                Err(e) => {
                    warn!("sonar[{}]: cannot open ECHO pin BCM{} ({e}) — sensor skipped",
                ...
                .spawn(move || run(state, cfg, trig, echo, ready))
            {
                Ok(h) => Some(h),
                Err(e) => {
                    warn!("sonar[{}]: failed to spawn thread ({e}) — sensor skipped", cfg.label);
...
fn run(
    ...
    info!("sonar[{}]: TRIG=BCM{} ECHO=BCM{} slot={}", cfg.label, cfg.trig_bcm, cfg.echo_bcm, cfg.slot);
    ...
    if let Err(e) = echo.set_async_interrupt(Trigger::Both, move |level| {
        ...
        warn!("sonar[{}]: set_async_interrupt failed ({e}) — sensor skipped", cfg.label);
    ...
    loop {
        ...
        debug!("sonar[{}]: {:.3} m", cfg.label, if dist_m >= DIST_MAX_M { f32::INFINITY } else { dist_m });
...
fn measure(
    ...
    let rise = loop {
        ...
        if r != prev_rise && r >= trigger_us { break r; }
    ...
    if dist_m < DIST_MIN_M || dist_m > DIST_MAX_M { DIST_MAX_M } else { dist_m }
}
