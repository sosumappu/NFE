...
mod linux_hw {
    ...
    fn run_imu(once: bool) {
        ...
        let mut i2c = match I2c::new() {
            Ok(i) => i,
            Err(e) => {
                fail(&format!("cannot open /dev/i2c-1: {e}"));
            }
        };
        ...
        if who != 0x68 && who != 0x70 && who != 0x19 {
            fail(&format!(
                "unexpected WHO_AM_I=0x{who:02X} — wrong device or address"
            ));
        }
        ...
        loop {
            ...
            if once {
                ...
                if gravity_ok && noise_ok {
                    ...
                    fail(&format!(
                        "az={az:.2} m/s² — expected ≈ ±9.81 (is the IMU flat? is it powered?)"
                    ));
                } else {
                    fail(&format!(
                        "ax={ax:.2} ay={ay:.2} — high lateral noise (loose connection?)"
                    ));
                }
                return;
            }
    ...
    fn run_lidar(once: bool) {
        ...
        println!(
            "  {:>5} {:>8} {:>8} {:>10} {:>10}",
            "rev", "points", "min_m", "front_m", "elapsed_s"
        );
        ...
        loop {
            ...
            if start_flag && !prev_start && rev > 0 {
                ...
                if once && rev >= 1 {
                    ...
                    if rev > 0 && min_dist < 6.0 {
                        ...
                        fail(
                            "LIDAR returned no valid points — check motor power and USB connection",
                        );
                    }
                    return;
                }
            ...
            if quality > 0 && dist_m > 0.1 && dist_m < 6.0 {
                ...
                if dist_m < sectors[sector] {
                    sectors[sector] = dist_m;
                }
            }
        }
    }
    ...
    fn run_sonar(once: bool) {
        ...
        let gpio = match Gpio::new() {
            Ok(g) => g,
            Err(e) => {
                fail(&format!("GPIO init failed: {e}"));
            }
        };
        ...
        for _ in 0..iterations {
            ...
            if once {
                ...
                if all_ok {
                    ...
                    fail("one or more sonars timed out");
                }
                return;
            }
...
fn main() {
    println!("car-diag is a hardware diagnostic tool for the car.");
    println!("It cannot be run on macOS. Please run this on the Raspberry Pi!");
}
